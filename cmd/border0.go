...
const (
    ...
    border0:
      ...
      binds:
        ...
        - /etc/border0/border0.yaml:/etc/border0/border0.yaml
`
...
var border0InitCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		config := border0NodeClabConfigWithTokenVolume
		if err = writeConnectorConfig("/etc/border0/border0.yaml", connectorToken.Token); err != nil {
			fmt.Printf("Warning: failed to write Border0 connector configuration file (will use token env): %v\n", err)
...
func writeConnectorConfig(filePath, token string) error {
	...
	if err := os.WriteFile(filePath, []byte(fmt.Sprintf("token: %s", token)), 0644); err != nil {
		return err
	}
	return nil
}
