...
const (
	border0NodeClabConfigWithTokenVolume = `
    border0:
      kind: linux
      ...
      binds:
        ...
        - /etc/border0/config.yaml:/etc/border0/config.yaml
`
...
var border0InitCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err = writeConnectorConfig("/etc/border0/border0.yaml", connectorToken.Token); err != nil {
			fmt.Println("Add the following configuration to your *.clab.yaml file:")
			fmt.Println(border0NodeClabConfigWithTokenVolume)
		} else {
			fmt.Println("NOTE: failed to write connector configuration, will use config with token in plaintext")

			fmt.Println("Add the following configuration to your *.clab.yaml file:")
			fmt.Println(fmt.Sprintf(border0NodeClabConfigWithTokenEnvFmt, connectorToken.Token))
		}
		return nil
	},
}

func writeConnectorConfig(filenpath, token string) error {
	dir := filepath.Dir(filenpath)
	if err := os.MkdirAll(dir, 0755); err != nil {
		return err
	}
	if err := os.WriteFile(filenpath, []byte(fmt.Sprintf("token: %s", token)), 0644); err != nil {
		return err
	}
	return nil
}
